...
`OutboxOptions`: Retries=3, MessagesToProcess=100, ConcurrencyLimit=1, GetFreeMessagesTimeout=20s, MaxContentLength=1 МиБ. `AddOutbox` регистрирует инфраструктуру опций (IOptions резолвится), но правила OutboxOptions на старте НЕ валидируются: `OutboxOptionsValidator` не подключён (в отличие от Inbox, это известный технический долг). Значения задают через `services.Configure<OutboxOptions>(...)`.

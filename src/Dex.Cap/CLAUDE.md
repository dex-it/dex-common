...
- Inbox: коллизия дискриминаторов роняет СТАРТ хоста (`DiscriminatorConflictException` из warm-up hosted service, регистрируется в `AddInbox`), в отличие от Outbox, который молча берёт первый тип. Пустой дискриминатор и кавычка в нём тоже ловятся на старте (`DiscriminatorResolveException`)

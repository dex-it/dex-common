...
`OutboxOptions`: Retries=3, MessagesToProcess=100, ConcurrencyLimit=1, GetFreeMessagesTimeout=20s, MaxContentLengthBytes=1 МиБ. `AddOutbox` регистрирует инфраструктуру опций и ЕДИНСТВЕННОЕ правило: `MaxContentLengthBytes > 0` (`OutboxMaxContentLengthBytesValidator`, взводится `ValidateOnStart`). Правило заведено вместе с опцией, спящих конфигов у него нет. Остальные правила из `OutboxOptionsValidator` не исполняются: валидатор не подключён, его включение отвергло бы конфиги, годами принимавшиеся молча. Отвергаемые конфиги есть в самом репозитории, оба в тестах: `GetFreeMessagesTimeout=10ms` (`Dex.Events.Distributed.Tests/BaseTest.cs`) и `ConcurrencyLimit=2 > MessagesToProcess=1` (`ExecuteTransactionOutboxTests.ParallelProcessingMessagesTest`). Это известный технический долг, вынесен в issue #239. ВАЖНО: `ValidateOnStart` взводится на ЭКЗЕМПЛЯР опций (тип плюс имя, здесь имя по умолчанию), а не на валидатор, поэтому на старте хоста исполняются ВСЕ зарегистрированные `IValidateOptions<OutboxOptions>`, включая регистрации потребителя. Прогон делает `OptionsFactory` при СОЗДАНИИ значения, а не при каждом чтении `.Value`: для `IOptions` это один раз за процесс, для `IOptionsSnapshot` раз на scope. Значения задают через `services.Configure<OutboxOptions>(...)`.
`OutboxHandlerOptions`: Period=30s, CleanupInterval=1h, CleanupOlderThan=30d.
`InboxOptions`: Retries=3, MessagesToProcess=25, ConcurrencyLimit=1, GetFreeMessagesTimeout=20s (минимум 1s: таймаут команды задаётся целыми секундами, доля усекается в ноль, а ноль означает «не задан»), MaxContentLengthBytes=1 МиБ. Валидируются на старте хоста (`ValidateOnStart`): неверная конфигурация роняет запуск.
...
Полный набор требует трёх контейнеров; CI поднимает их сам:
```bash
...
- `Content` ограничен опцией `MaxContentLengthBytes` (дефолт 1 МиБ, в БАЙТАХ UTF-8, меряется как HTTP Content-Length), симметрично в Outbox и Inbox. Проверка на постановке/приёме (`OutboxService`/`InboxService.EnqueueAsync`), до записи в БД: превышение бросает `OutboxContentTooLargeException`/`InboxContentTooLargeException`. Жёсткого `HasMaxLength` на колонке нет намеренно (не навязывать миграцию). Публичные конструкторы `OutboxEnvelope`/`InboxEnvelope` + прямая вставка конверта через свой `DbContext` в обход `EnqueueAsync` предел не проверяют: это сознательное следствие отказа от БД-констрейнта (сами `IOutboxDataProvider`/`IInboxDataProvider` — `internal`, снаружи их не вызвать)

...
`AddInbox` даёт приём и обработку; `IInboxCleanupDataProvider` и фоновые сервисы регистрирует ТОЛЬКО `AddDefaultInboxScheduler`/`AddInboxScheduler`. Синглтоны инбокса идут через `TryAdd` (побеждает регистрация ДО `AddInbox`), scoped-сервисы, включая `IInboxSerializer`, — обычным `Add` (побеждает регистрация ПОСЛЕ).
...
`InboxOptions`: Retries=3, MessagesToProcess=100, ConcurrencyLimit=1, GetFreeMessagesTimeout=20s. Валидируются на старте хоста (`ValidateOnStart`): неверная конфигурация роняет запуск.
...
Ретрай-стратегии инбокса: `DefaultInboxRetryStrategy` (без задержки), `IncrementalInboxRetryStrategy(interval)`, `ExponentialInboxRetryStrategy(baseDelay, maxDelay)` — задержка `baseDelay * 2^(N-1)`, ограничена `maxDelay`. Задержка отсчитывается от момента ОТКАЗА, а не от прежнего `StartAtUtc`: иначе на отстающей обработке все попытки сгорают мгновенно.
...
Полный набор (187 тестов) требует трёх контейнеров; CI поднимает их сам:
```bash
...
- Inbox: тело сообщения лежит в БД и читается ПОСЛЕ деплоя, поэтому схема сообщения это контракт. Дефолтный `System.Text.Json`: enum пишутся ЧИСЛАМИ (перестановка членов молча меняет смысл сохранённых строк), переименование поля или смена типа делает тело нечитаемым (ретраи, затем DeadLettered), добавление поля безопасно. Меняется через свой `IInboxSerializer`

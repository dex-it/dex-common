    ...
    where TDbContext : DbContext
{
    ...
    /// </summary>
    /// <remarks>
    /// Свой таймаут команды тут намеренно НЕ задаётся, в отличие от захвата партии
    /// (<c>InboxOptions.GetFreeMessagesTimeout</c>) и от фиксации исхода (<c>EfTransactionOptions.Default</c>).
    /// Те два пути ограничены арендой: захват, переживший её, отдаёт партию, которую уже некому дренировать, а
    /// фиксация, пережившая её, пишет в чужую строку. Здесь аренды нет, и дедлайна, от которого можно было бы
    /// отсчитать своё число, тоже нет. За строки чистка не борется с выборкой: удаляются только Succeeded, то
    /// есть строки с пустым ScheduledStartIndexing, которых выборка не видит. Значит, единственная осмысленная
    /// граница это таймаут команды самого потребителя, и перебивать его нам нечем: своё число было бы выдумано,
    /// а лишняя настройка описывала бы несуществующее ограничение.
    /// <para>
    /// Оговорка на будущее: с ПРИЁМОМ чистка формально пересекается, потому что делит с ним уникальный индекс
    /// (MessageId, ConsumerId), и удаление строки может подержать вставку того же ключа. Практического веса это
    /// не имеет: чистка трогает только строки старше ретеншена (по умолчанию 30 суток), а редоставка такого
    /// возраста означает, что окно дедупликации и так истекло.
    /// </para>
    /// </remarks>

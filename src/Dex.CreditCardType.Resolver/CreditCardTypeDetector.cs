...
namespace Dex.CreditCardType.Resolver
{
    ...
    public static partial class CreditCardTypeDetector
    {
        public static CardType FindType(string cardNumber)
        {
            ...
            if (MirRegex().Match(cardNumber).Success)
                return CardType.Mir;

            if (AmericanExpressRegex().Match(cardNumber).Success)
                return CardType.AmericanExpress;

            if (DiscoverRegex().Match(cardNumber).Success)
                return CardType.Discover;

            if (JcbRegex().Match(cardNumber).Success)
                return CardType.JCB;

            if (DinersClubRegex().Match(cardNumber).Success)
                return CardType.DinersClub;

            if (MaestroRegex().Match(cardNumber).Success)
                return CardType.Maestro;

            if (UnionPayRegex().Match(cardNumber).Success)
                return CardType.UnionPay;

            throw new InvalidDataException("Unknown card") {Data = {{"number", cardNumber}}};
        }
        ...
        [GeneratedRegex(@"^(?:220[0-4])\d{12}$")]
        private static partial Regex MirRegex();

        [GeneratedRegex("^3[47][0-9]{13}$")]
        private static partial Regex AmericanExpressRegex();

        [GeneratedRegex("^65[4-9][0-9]{13}|64[4-9][0-9]{13}|6011[0-9]{12}|(622(?:12[6-9]|1[3-9][0-9]|[2-8][0-9][0-9]|9[01][0-9]|92[0-5])[0-9]{10})$")]
        private static partial Regex DiscoverRegex();

        [GeneratedRegex(@"^(?:2131|1800|35\d{3})\d{11}$")]
        private static partial Regex JcbRegex();

        [GeneratedRegex("^3(?:0[0-5]|[68][0-9])[0-9]{11}$")]
        private static partial Regex DinersClubRegex();

        [GeneratedRegex("^(50|56|57|58|60|61|62|63|64|65|66|67|68|69)[0-9]{10,17}$")]
        private static partial Regex MaestroRegex();

        [GeneratedRegex("^(62[0-9]{14,17})$")]
        private static partial Regex UnionPayRegex();
    }
    ...
    public static class CreditCardTypeDetector
    {
        public static CardType FindType(string cardNumber)
        {
            ...
            if (Regex.Match(cardNumber, "^65[4-9][0-9]{13}|64[4-9][0-9]{13}|6011[0-9]{12}|(622(?:12[6-9]|1[3-9][0-9]|[2-8][0-9][0-9]|9[01][0-9]|92[0-5])[0-9]{10})$")
                .Success)
                return CardType.Discover;
